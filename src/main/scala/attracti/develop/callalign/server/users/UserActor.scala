...
class UserActor(val user: User) extends Actor {
...
//  val user:User= usr
  ...
  user.log= log
  user.ex=context.system.dispatcher
...
//    log.info("Start new UserActor "+actorName)
  ...
  override def receive = {
    LoggingReceive {
      ...
      case TcpToUserSetIIndex(str, pid) => user.updIIndex(str, pid)
      ...
      case IntentManagerToUserAddIntentToRecycle(int, inOut) => {
        inOut match {
          case 0 => {
            user.recycleIncomingIntents += (int.iid -> int)
          }
          case 1 => {
            user.recycleOutgoingIntents += (int.iid -> int)
          }
        }
      }
      ...
      case IntentManagerToUserAddIntent(int, inOut) => {
        inOut match {
          case 0 => {
            user.incomingIntent += (int.iid -> int)
          }
          case 1 => {
            user.outgoingIntent += (int.iid -> int)
          }
        }
      }
      ...
      case TcpToUserNewConnection(aRef) => {
        ...
        if (connection != null) {
          ...
          if (aRef != null) {
            connection ! UserToTcpCloseConnection()
          }
        }
